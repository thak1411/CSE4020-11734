...
def hermite(t, a, b, u, v):
    s = 1 - t
    return s ** 2 * (1 + 2 * t) * a + t ** 2 * (1 + 2 * s) * b + s ** 2 * t * u - s * t ** 2 * v

def catmull_rom(t, a, b, c, d):
    u = (c - a) / 2
    v = (d - b) / 2
    return hermite(t, b, c, u, v)

def bspline(t, a, b, c, d):
    t1 = t ** 3
    t2 = t ** 2
    t = t
    ...
    b1 = (-t1 + 3 * t2 - 3 * t + 1) / 6
    b2 = (3 * t1 - 6 * t2 + 4) / 6
    b3 = (-3 * t1 + 3 * t2 + 3 * t + 1) / 6
    b4 = t1 / 6
    
    return b1 * a + b2 * b + b3 * c + b4 * d

def display():
    global cameraIndex, cow2wld, rn_state, control_points, animStartTime
    ...
    if rn_state == 6: # animation start
        rn_state = 7
        animStartTime = glfw.get_time()

    if rn_state == 7: # do animation
        animTime = (glfw.get_time() - animStartTime) # current animation time
        cp = control_points
        cl = len(cp)

        x = int(np.floor(animTime))
        t = animTime - x
        if x >= 3 * cl: # animation end
            rn_state = -1
            cow2wld = control_points[0].copy()
            control_points = []
            return
        ...
        a, b, c, d = cp[x - 1], cp[x], cp[(x + 1) % cl], cp[(x + 2) % cl] # four consecutive control points from b to c

        # if you want to use catmull-rom spline, uncomment the following line and comment the next line
        # cur = catmull_rom(t, a, b, c, d)
        # nxt = catmull_rom(t + 0.0001, a, b, c, d)

        cur = bspline(t, a, b, c, d)
        nxt = bspline(t + 0.0001, a, b, c, d)

        forward = getTranslation(nxt - cur) # tangent vector

        inv = np.linalg.inv(cur)
        glMultMatrixd(cur.T) # set cow's position to world coordinate
        glRotatef(np.arctan2(forward[0], forward[2]) * 180 / np.pi, 0, 1, 0)
        glRotatef(np.arctan2(forward[1], np.sqrt(forward[0] ** 2 + forward[2] ** 2)) * 180 / np.pi, 0, 0, 1)
        glMultMatrixd(inv.T) # revert cow's position to local coordinate
        drawCow(cur, False)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, rn_state, control_points, pickInfo
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            if cursorOnCowBoundingBox:
                if rn_state >= 6: return # if do animation state then ignore
                if rn_state == -1: # if initial state then start horizontal dragging
                    rn_state = 0
                    isDrag = H_DRAG
                    return
            ...
            if cursorOnCowBoundingBox:
                if (rn_state == 0 and isDrag == H_DRAG) or rn_state >= 6: return # if initial state or do animation state then ignore
                control_points.append(pp.cowPickConfiguration) # save the point of currnet cow's configuration
                rn_state += 1 # increase the state
                if rn_state >= 6: isDrag = 0 # is capture the all of cow's configuration then stop dragging
                else: isDrag = H_DRAG # else continue horizontal dragging
            print( "Left mouse up\n");
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, rn_state
    if isDrag:
        if rn_state == 7: return
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                T=np.eye(4)
                setTranslation(T, vector3(0, (currentPos-pp.cowPickPosition)[1], 0)) # set translation only y-axis
                cow2wld=T@pp.cowPickConfiguration;
                pp.cowPickConfiguration = cow2wld.copy()
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, '2020021949-PA2', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
